...
def to_signature(h):
    """Arrnages the histogram into the format required by cv2.EMD"""
    idx = np.argwhere(h > 0)
...
def hist_diff(h1, h2):
    """Calculates Earth Mover's Distance between h1 and h2"""
    bins = h1.shape[0]
    sig1 = to_signature(h1)
    sig2 = to_signature(h2)
    dist, _, _ = cv2.EMD(sig1, sig2, cv2.DIST_L2)
    max_dist = np.sqrt(2) * (bins - 1)
    return float(np.clip(dist / max_dist, 0.0, 1.0)) # normalized
...
class FeatureExtractor:
    ...
    def __call__(self, image_path, visualize=False):
        ...
        visualization = None
        if visualize:
            visualization = (r, patch_color_signature, upper_seam_color_signature, lower_seam_color_signature, valid_char_count, chars_color_signatures, chars_cropped)

        return (r_ratio, patch_color_diff, upper_seam_color_diff, lower_seam_color_diff, valid_char_count, chars_color_diff, chars_structural_diff), visualization
